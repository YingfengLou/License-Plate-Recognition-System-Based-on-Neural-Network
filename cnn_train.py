...
#用于将矩阵随机划分为训练子集和测试子集，并返回划分好的训练集测试集样本和训练集测试集标签。（X_train,X_test, y_train, y_test）
...
class char_cnn_net:
    ...
    def cnn_construct(self):
        x_input = tf.reshape(self.x_place, shape=[-1, 20, 20, 1])#将一组图像矩阵重建为新的矩阵
        #卷积核高度=宽度=3，通道数=1，个数=32
        ...
        conv1 = tf.nn.relu(tf.nn.bias_add(tf.nn.conv2d(x_input,filter=cw1,strides=[1,1,1,1],padding='SAME'),cb1))#卷积，输入图像，卷积核，图像每一维的步长，卷积方式为samae
        conv1 = tf.nn.max_pool(conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')#使用最大池化：卷积层的输出即为输入，池化窗口大小（ksize)，窗口在每一个维度上滑动的步长，
...
if __name__ == '__main__':
    ...
    model_path = os.path.join(cur_dir,'./carIdentityData/model/char_recongnize/model.ckpt-560')
